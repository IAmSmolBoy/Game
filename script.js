...
function FindPipeHeight(i,PipeType,passed){
    if(passed===false&&PipeType==='Pipe'){
        return parseInt(PipeList[i-1].slice(PipeList[i-1].length-2,PipeList[i-1].length))
    }
    if(passed===false&&PipeType==='Other'){
        return 80-parseInt(PipeList[i-1].slice(PipeList[i-1].length-2,PipeList[i-1].length))
    }if(passed===true&&PipeType==='Pipe'){
        return parseInt(PipesPassed[i-1].slice(PipesPassed[i-1].length-3,PipesPassed[i-1].length-1))
    }
    if(passed===true&&PipeType==='Other'){
        return 80-parseInt(PipesPassed[i-1].slice(PipesPassed[i-1].length-3,PipesPassed[i-1].length-1))
    }
}
...
function CollisionTest(){
    setTimeout(function(){
        for(i=1;i<=PipeList.length;i++){
            var PipeY=FindPipeHeight(i,'Pipe',false)
...
function Movementpassed(){
    if(PipesPassed.length!=0){
        for(i=1;i<=PipesPassed.length;i++){
            var PipeY=FindPipeHeight(i,'Pipe',true)
            ...
            if(posX>=parseInt(document.getElementById(`${PipesPassed[i-1]}`).style.left)&&posY/document.documentElement.clientHeight*100<PipeY&&posX<parseInt(document.getElementById(`${PipesPassed[i-1]}`).style.left)+25){
                if(score>Highscore){
                    Highscore=score
                }
            document.body.innerHTML=`<div id="gameover">Game Over</div>
        ...
        if(posX>=parseInt(parseInt(document.getElementById(`${PipesPassed[i-1]}`).style.left))&&posY/document.documentElement.clientHeight*100>20+PipeY&&posX<parseInt(parseInt(document.getElementById(`${PipesPassed[i-1]}`).style.left))+25){
            if(score>Highscore){
                Highscore=score
            }
            document.body.innerHTML=`<div id="gameover">Game Over</div>
